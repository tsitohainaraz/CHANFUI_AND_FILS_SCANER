...
# ---------------------------
def bdc_pipeline(text):
    """
    Extraction structurée d’un Bon de Commande depuis le texte brut Google Vision.
    """
    ...
    for l in lines:
        if "/" in l and len(l) <= 10:
            data["date_emission"] = l
            break

    # ---- 2. Numéro de bon de commande ----
    for l in lines:
    ...
    start = False
    for l in lines:
        if "Adresse livraison" in l:
            start = True
            continue
        if start:
            if l.startswith("Adresse facturation"):
                break
            adr_liv.append(l)
    ...
    start = False
    for l in lines:
        if "Adresse facturation" in l:
            start = True
            continue
        if start:
            if "Adresse fournisseur" in l:
                break
            adr_fac.append(l)
    data["adresse_facturation"] = "\n".join(adr_fac)

    # ---- 5. Tableau désignation + quantité ----
    start = False
    for l in lines:
        if "Désignation" in l:
            start = True
            continue
        if start:
            if "Horaire" in l or "Montant" in l:
                break

            # Exemple : "vin de madagascar 75 cl blanc" puis "12 ,000"
            if l.replace(",", "").replace(".", "").isdigit():
                ...
                data["items"].append({"designation": l, "qty": ""})
...
# ---------------------------
# Interface Streamlit complète (UI vaovao)
# ---------------------------
def interface_bdc(ocr_text, data):
    st.markdown("<h3 style='text-align:center;'>Les informations détectée</h3>", unsafe_allow_html=True)

    # --- Bloc Info 1 ---
    st.write("### Info 1 : Date d’emission")
    st.text_input("", data["date_emission"], key="info_date")

    # --- Bloc Info 2 ---
    st.write("### Info 2 : Numero de bon de commande")
    st.text_input("", data["numero_bdc"], key="info_num")

    # --- Bloc Info 3 ---
    st.write("### Info 3 : Adresse livraison")
    st.text_area("", data["adresse_livraison"], key="info_adr_liv", height=80)

    # --- Bloc Info 4 ---
    st.write("### Info 4 : Adresse facturation")
    st.text_area("", data["adresse_facturation"], key="info_adr_fac", height=60)

    # --- Tableau désignation + quantité ---
    st.write("### Info 5 - 6 : designation et Quantité")

    df_items = pd.DataFrame(data["items"])
    st.table(df_items)

    if st.button("+ Ajouter un champs"):
        data["items"].append({"designation": "", "qty": ""})
        st.experimental_rerun()

    # --- Données brutes OCR ---
    st.write("### Les donnees brutes du bon de commande par Google Vision")
    st.text_area("", ocr_text, height=250)

    # --- Boutons ---
    col1, col2 = st.columns(2)
    with col1:
        if st.button("Exporter dans sheet"):
            st.success("Exporté !")
            # ICI → appeler ta fonction Google Sheets d’insertion
...
# VAOVAO ajoute
#---------------------------
if choice == "Bon de commande":
    uploaded = st.file_uploader("Importer le bon de commande", type=["jpg", "png", "jpeg", "pdf"])
    ...
    if uploaded:
        ...
        data = bdc_pipeline(ocr_text)
        interface_bdc(ocr_text, data)
